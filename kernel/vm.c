...
// map the kernel page table to itself
...
void vvmprintf(pagetable_t pagetable, int depth)
{
  for(int i = 0; i < 512; i++)
  {
    pte_t pte = pagetable[i];
    if(pte & PTE_V)
    {
      ...
      if((pte & (PTE_R|PTE_W|PTE_X)) == 0) 
        vvmprintf((pagetable_t)pa, depth + 1);
    } 
  }
}
...
{
  printf("page table %p\n", pagetable);
  vvmprintf(pagetable, 0); 
}
int
copymappings(pagetable_t src,pagetable_t dst, uint64 start, uint64 sz){
  pte_t *pte;
  uint64 pa, i;
  uint flags;
